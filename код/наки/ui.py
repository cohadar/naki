...
class UI():
    ...
    def одабери_шпил(бре, приказ, порука, екстра):
        return бре._одабери_један(приказ, порука, екстра)

    def _одабери_један(бре, листа, опис, екстра=None):
        while True:
            бре.т.обриши()
            with бре.т.статус():
            ...
            with бре.т.главни():
                ...
                индекс = бре._инпут_индекс(len(листа), екстра=екстра)
                if индекс == -1:
                    бре.т.звук_грешке()
                else:
                    return индекс

    def _инпут_индекс(бре, број_елемента, екстра=None):
        индекс = бре.т.инпут('#? ')
