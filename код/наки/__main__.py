...
class ШпилКонтејнер(containers.DynamicContainer):
    def __init__(к, отац, дир):
        ...
        к.ui_стање = providers.Singleton(UIСтање)  # локални синглтон
        к.питање_ui = providers.Factory(
            ПитањеUI, терминал=к.отац.терминал, регистар=к.отац.регистар, стање=к.ui_стање)
        к.одговор_ui = providers.Factory(
            ОдговорUI, терминал=к.отац.терминал, регистар=к.отац.регистар, стање=к.ui_стање)

        к.шпил = providers.Factory(
            Шпил,
            питање_ui=к.питање_ui,
            одговор_ui=к.одговор_ui,
            дир=к.дир,
