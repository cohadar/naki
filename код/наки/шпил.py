...
class Шпил():
    def __init__(бре, ui, дир, п_интервала, п_карти, п_линкова, п_записа):
        ...
        регистар_одговор.региструј(КОМАНДА_ФЕЈЛ, бре.а_оцена_фејл)
        регистар_одговор.региструј(КОМАНДА_ПЛУС1, бре.а_оцена_плус1)
        регистар_одговор.региструј(КОМАНДА_ПЛУС2, бре.а_оцена_плус2)
    ...
    def а_прекид(бре, **дата):
        бре.прекид = True
        бре.понови_питање = False
        бре.понови_одговор = False

    def а_оцена_фејл(бре, ид, **дата):
        бре.оцена(ид, 0)

    def а_оцена_плус1(бре, ид, **дата):
        бре.оцена(ид, 1)

    def а_оцена_плус2(бре, ид, **дата):
        бре.оцена(ид, 2)

    def а_отвори_урл(бре, урл, **дата):
        бре._ui.отвори_урл(урл)

    def а_измени(бре, извор, линија, **дата):
