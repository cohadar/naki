...
def шпил_лоадер(дир, лен):
    ...
    for извор_фајл in ['aaa.tsv', 'bbb.tsv', 'ccc.tsv']:
        ...
        линкови.extend(л)
    карте = карте[:лен]
    п_карти = ПогледКарти(ТестТабела(Path('карте.tsv'), карте))
    assert len(п_карти) == лен, 'поглед мора одма да учита табелу'
    интервали = []
    for врста_карте in п_карти.скуп_врста_карте:
    ...
    return {
        "име": дир.name,
        "п_интервала": ПогледИнтервала(ТестТабела(Path('интервали.tsv'), интервали)),
        "п_карти": п_карти,
        "п_линкова": ПогледЛинкова(ТестТабела(Path('линкови.tsv'), линкови)),
        "п_записа": ПогледЗаписа(ТестТабела(Path('запис.tsv'), запис))
    }
