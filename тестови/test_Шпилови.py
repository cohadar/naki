...
def тт(команде):
    главна = ТестКонтејнер(команде, [ПУН_ШПИЛ]).главна()
    return главна.шпилови[0]


def test_одради_све():
    в = тт(['СЛЕДЕЋИ', КОД_СПЕЈС] * 10000)
...
def test_изађи_на_питању():
    в = тт(['СЛЕДЕЋИ', КОД_СПЕЈС] * 2 + [КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
...
def test_изађи_на_одговору():
    в = тт(['СЛЕДЕЋИ', КОД_СПЕЈС, 'СЛЕДЕЋИ', КОД_СПЕЈС, 'СЛЕДЕЋИ'] + [КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
    assert неодрађене == ПУН_ШПИЛ - 2
...
def test_едит_на_питању():
    в = тт([КОД_ЦИР_Е, КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
    assert неодрађене == ПУН_ШПИЛ
    assert промашене == 0
    assert len(в._ui.т.измене) == 1


def test_едит_на_одговору():
    в = тт([КОД_СПЕЈС, КОД_ЦИР_Е, КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
    assert неодрађене == ПУН_ШПИЛ
...
def test_линк_на_питању():
    в = тт([КОД_ЦИР_О, КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
    assert неодрађене == ПУН_ШПИЛ
    assert промашене == 0
    assert len(в._ui.т.отварања) == 1


def test_линк_на_одговору():
    в = тт([КОД_СПЕЈС, КОД_ЦИР_О, КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
    assert неодрађене == ПУН_ШПИЛ
...
def test_недозвољенe_командe():
    в = тт([КОД_СПЕЈС, 'џ', 'љ', КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
    assert неодрађене == ПУН_ШПИЛ
    assert промашене == 0
    assert в._ui.т.звукова == 2


def test_неколико_прегледа():
    в = тт(['СЛЕДЕЋИ', КОД_СПЕЈС] * 5 + ['СЛЕДЕЋИ', КОД_ЦИР_Ф] * 10 + ['СЛЕДЕЋИ', КОД_ЕНТЕР] * 3 + [КОД_ЦИР_К])
    assert len(в) == ПУН_ШПИЛ
    неодрађене, промашене = в()
    assert len(в) == 0
