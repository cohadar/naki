...
ТипКолоне = namedtuple('ТипКолоне', [
    'ИД', 'ДАТУМ', 'ИНФИНИТИВ', 'ПРЕЗЕНТ', 'ПРЕТЕРИТ', 'ПЕРФЕКТ', 'ПРЕВОД'
])
...
ТипКолонеПогрешнаВеличина = namedtuple('ТипКолонеПогрешнаВеличина', [
    'ИД', 'ДАТУМ', 'ИНФИНИТИВ', 'ПРЕЗЕНТ', 'ПРЕТЕРИТ', 'ПЕРФЕКТ'
])
...
ТипКолонеПогрешноИме = namedtuple('ТипКолонеПогрешноИме', [
    'ИД', 'ДАТУМ', 'ИНФИНИТИВ', 'ПРЕЗЕНТЛАЛАЛА', 'ПРЕТЕРИТ', 'ПЕРФЕКТ', 'ПРЕВОД'
])
...
def test_учитај_КолонеПогрешнаВеличина():
    path = Path('тест-фајлови/tsv/ok10.tsv')
    with pytest.raises(tsv.КолонеПогрешнаВеличинаГрешка):
        Табела(path, ТипКолонеПогрешнаВеличина).учитај()
...
def test_учитај_КолонеИменаСеНеПоклапају():
    path = Path('тест-фајлови/tsv/ok10.tsv')
    with pytest.raises(tsv.КолонеИменаСеНеПоклапајуГрешка):
        Табела(path, ТипКолонеПогрешноИме).учитај()
...
def test_додај_на_постоји(tmpdir):
    copy('тест-фајлови/tsv/ok08.tsv', tmpdir)
    path = Path(tmpdir).joinpath('ok08.tsv')
    Табела(path, ТипКолоне).додај(РЕДОВИ)
    assert фајл_хеш('тест-фајлови/tsv/ok10.tsv') == фајл_хеш(path)
...
def test_додај_на_непостојећи(tmpdir):
    path = Path(tmpdir).joinpath('ok_ново.tsv')
    Табела(path, ТипКолоне).додај(РЕДОВИ)
    assert фајл_хеш('тест-фајлови/tsv/ok02.tsv') == фајл_хеш(path)
...
def test_додај_КолонеПогрешнаВеличина(tmpdir):
    copy('тест-фајлови/tsv/ok10.tsv', tmpdir)
    path = Path(tmpdir).joinpath('ok10.tsv')
    with pytest.raises(tsv.КолонеПогрешнаВеличинаГрешка):
        Табела(path, ТипКолонеПогрешнаВеличина).додај(РЕДОВИ)
...
def test_додај_КолонеИменаСеНеПоклапају(tmpdir):
    copy('тест-фајлови/tsv/ok10.tsv', tmpdir)
    path = Path(tmpdir).joinpath('ok10.tsv')
    with pytest.raises(tsv.КолонеИменаСеНеПоклапајуГрешка):
        Табела(path, ТипКолонеПогрешноИме).додај(РЕДОВИ)
...
def test_препиши_нови(tmpdir):
    copy('тест-фајлови/tsv/ok10.tsv', tmpdir)
    фајл1 = Path(tmpdir).joinpath('ok10.tsv')
    фајл2 = Path(tmpdir).joinpath('ok10_нови.tsv')
    редови = Табела(фајл1, ТипКолоне).учитај()
    Табела(фајл2, ТипКолоне).препиши(редови)
    assert фајл_хеш(фајл1) == фајл_хеш(фајл2)
...
def test_препиши_постојећи(tmpdir):
    copy('тест-фајлови/tsv/ok10.tsv', tmpdir)
    copy('тест-фајлови/tsv/ok02.tsv', tmpdir)
    фајл1 = Path(tmpdir).joinpath('ok10.tsv')
    фајл2 = Path(tmpdir).joinpath('ok02.tsv')
