...
def главна(команде):
    return ТестКонтејнер(команде, [0, 5, 10, 15]).главна()
...
def test_одради_цео_шпил():
    рез = главна(['2'] + (['СЛЕДЕЋИ', КОД_СПЕЈС] * 5) + [КОД_ЦИР_К])()
    assert рез == [
        (0, 0),
        (ОДРАЂЕНО, 0),
        (10, 0),
...
def test_одради_двe_карте():
    рез = главна(['2'] + (['СЛЕДЕЋИ', КОД_СПЕЈС] * 2) + [КОД_ЦИР_К] * 2)()
    assert рез == [
        (0, 0),
        (5 - 2, 0),
        (10, 0),
...
def test_одради_два_шпила():
    рез = главна(['2'] + (['СЛЕДЕЋИ', КОД_СПЕЈС] * 5) + ['3'] + (['СЛЕДЕЋИ', КОД_ЕНТЕР] * 10) + [КОД_ЦИР_К])()
    assert рез == [
        (0, 0),
        (ОДРАЂЕНО, 0),
        (ОДРАЂЕНО, 0),
